...
namespace Gs2.Gs2Deploy.Result
{
	...
	public class DescribeEventsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeEventsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeEventsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Deploy.Model.Event[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Deploy.Model.Event.FromJson(v);
                }).ToArray())
