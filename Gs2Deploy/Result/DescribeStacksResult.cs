...
namespace Gs2.Gs2Deploy.Result
{
	...
	public class DescribeStacksResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeStacksResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeStacksResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Deploy.Model.Stack[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Deploy.Model.Stack.FromJson(v);
                }).ToArray())
