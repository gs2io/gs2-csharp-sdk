...
namespace Gs2.Gs2Experience.Result
{
	...
	public class DescribeExperienceModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeExperienceModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeExperienceModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Experience.Model.ExperienceModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Experience.Model.ExperienceModelMaster.FromJson(v);
                }).ToArray())
