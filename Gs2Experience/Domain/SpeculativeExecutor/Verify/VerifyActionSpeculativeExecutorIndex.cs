...
/* diff +++ start */
        ...
        public static async Task<Func<object>> ExecuteInverseAsync(
            ...
            if (VerifyRankByUserIdSpeculativeExecutor.Action() == verifyAction.Action) {
                var request = VerifyRankByUserIdRequest.FromJson(JsonMapper.ToObject(verifyAction.Request));
                if (rate != 1) {
                    request = request.Rate(rate);
                }
                return await VerifyRankByUserIdSpeculativeExecutor.ExecuteInverseAsync(
                    domain,
                    accessToken,
                    request
                );
            }
            if (VerifyRankCapByUserIdSpeculativeExecutor.Action() == verifyAction.Action) {
                var request = VerifyRankCapByUserIdRequest.FromJson(JsonMapper.ToObject(verifyAction.Request));
                if (rate != 1) {
                    request = request.Rate(rate);
                }
                return await VerifyRankCapByUserIdSpeculativeExecutor.ExecuteInverseAsync(
                    domain,
                    accessToken,
                    request
                );
