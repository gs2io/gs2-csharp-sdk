...
{
    public static class SubRankCapByUserIdSpeculativeExecutor {
...
/* diff --- start
 ...
 diff --- end */
/* diff +++ start */
            var preparedRequest = SubRankCapByUserIdRequest.FromJson(
                request?.ToJson()
            );
...
/* diff +++ start */
            return await StatusSpeculativeExecutor.PrepareAsync(
                ...
                item => StatusSpeculativeExecutor.WithSubtractedRankCap(
                    item,
                    preparedRequest.RankCapValue.Value
                )
            );
/* diff +++ end */
        }
