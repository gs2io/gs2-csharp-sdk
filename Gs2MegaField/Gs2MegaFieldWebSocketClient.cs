...
namespace Gs2.Gs2MegaField
{
	public class Gs2MegaFieldWebSocketClient : AbstractGs2Client
	{
        ...
        public class GetAreaModelTask : Gs2WebSocketSessionTask<Request.GetAreaModelRequest, Result.GetAreaModelResult>
        {
	        public GetAreaModelTask(IGs2Session session, Request.GetAreaModelRequest request) : base(session, request)
	        {
	        }

            protected override IGs2SessionRequest CreateRequest(Request.GetAreaModelRequest request)
            {
                var stringBuilder = new StringBuilder();
                var jsonWriter = new JsonWriter(stringBuilder);
