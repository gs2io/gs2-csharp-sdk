...
namespace Gs2.Gs2MegaField.Result
{
	...
	public class DescribeLayerModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeLayerModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeLayerModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2MegaField.Model.LayerModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2MegaField.Model.LayerModelMaster.FromJson(v);
                }).ToArray())
