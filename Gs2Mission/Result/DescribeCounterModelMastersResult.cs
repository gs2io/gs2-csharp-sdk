...
namespace Gs2.Gs2Mission.Result
{
	...
	public class DescribeCounterModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeCounterModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeCounterModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Mission.Model.CounterModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Mission.Model.CounterModelMaster.FromJson(v);
                }).ToArray())
