...
namespace Gs2.Gs2Mission.Result
{
	...
	public class DescribeMissionTaskModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeMissionTaskModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeMissionTaskModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Mission.Model.MissionTaskModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Mission.Model.MissionTaskModelMaster.FromJson(v);
                }).ToArray())
