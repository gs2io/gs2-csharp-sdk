...
{
    public static class ResetCounterByUserIdSpeculativeExecutor {
            ...
            var prepared = request == null ? null :
                new ResetCounterByUserIdRequest()
                    ...
                    .WithScopes(request.Scopes?
                        ...
                        .Select(scope => scope.Clone() as ScopedValue)
                        .ToArray());
...
/* diff +++ start */
            ...
            var scopes = prepared.Scopes ?? Array.Empty<ScopedValue>();
            var commit = new CounterMutationSpeculativeCommit(
                ...
                current => {
                    ...
                    var retained = current.Values
                        .Where(value => value != null && !scopes.Any(scope =>
                            (string.IsNullOrEmpty(scope.ScopeType)
                                ? "resetTiming"
                                : scope.ScopeType) ==
                            (string.IsNullOrEmpty(value.ScopeType)
                                ? "resetTiming"
                                : value.ScopeType) &&
                            (scope.ResetType ?? "notReset") ==
                            (value.ResetType ?? "notReset") &&
                            (scope.ConditionName ?? "") ==
                            (value.ConditionName ?? "")
                        ))
                        .Select(value => value.Clone() as ScopedValue);
                    var reset = scopes.Select(scope => new ScopedValue {
                        ScopeType = string.IsNullOrEmpty(scope.ScopeType)
                            ? "resetTiming"
                            : scope.ScopeType,
                        ResetType = scope.ResetType ?? "notReset",
                        ConditionName = scope.ConditionName,
                        Value = 0,
                        NextResetAt = null,
                        UpdatedAt = logicalTimeMillis,
                    });
                    changed.Values = retained.Concat(reset).ToArray();
