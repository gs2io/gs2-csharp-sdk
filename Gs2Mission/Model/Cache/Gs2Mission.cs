...
namespace Gs2.Gs2Mission.Model.Cache
{
    public static class Gs2Mission
    {
        public static string PutUserData(
            ...
            switch (kind) {
                case "complete":
                    return Gs2.Gs2Mission.Model.Complete.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
                case "counter":
                    return Gs2.Gs2Mission.Model.Counter.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
            }
            return null;
        }
        ...
        public static bool SetListCached(
            ...
            switch (kind) {
                case "complete":
                    cache.SetListCached<Gs2.Gs2Mission.Model.Complete>(parentKey);
                    return true;
                case "counter":
                    cache.SetListCached<Gs2.Gs2Mission.Model.Counter>(parentKey);
                    return true;
            }
            return false;
        }
