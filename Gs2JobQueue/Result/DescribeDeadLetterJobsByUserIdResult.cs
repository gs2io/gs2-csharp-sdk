...
namespace Gs2.Gs2JobQueue.Result
{
	...
	public class DescribeDeadLetterJobsByUserIdResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeDeadLetterJobsByUserIdResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeDeadLetterJobsByUserIdResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2JobQueue.Model.DeadLetterJob[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2JobQueue.Model.DeadLetterJob.FromJson(v);
                }).ToArray())
