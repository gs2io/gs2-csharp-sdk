...
namespace Gs2.Gs2Buff.Result
{
	...
	public class DescribeBuffEntryModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeBuffEntryModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeBuffEntryModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Buff.Model.BuffEntryModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Buff.Model.BuffEntryModelMaster.FromJson(v);
                }).ToArray())
