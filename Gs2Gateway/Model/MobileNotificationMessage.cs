        ...
        public void Validate() {
            {
                if (Locale.Length > 32) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("mobileNotificationMessage", "gateway.mobileNotificationMessage.locale.error.tooLong"),
                    });
                }
            }
            {
                if (Title.Length > 256) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("mobileNotificationMessage", "gateway.mobileNotificationMessage.title.error.tooLong"),
                    });
                }
            }
            {
                if (Message.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("mobileNotificationMessage", "gateway.mobileNotificationMessage.message.error.tooLong"),
                    });
                }
            }
        }
