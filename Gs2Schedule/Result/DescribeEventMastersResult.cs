...
namespace Gs2.Gs2Schedule.Result
{
	...
	public class DescribeEventMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeEventMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeEventMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Schedule.Model.EventMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Schedule.Model.EventMaster.FromJson(v);
                }).ToArray())
