        ...
        public static Gs2Request ToRequest(Gs2.Core.Model.ConsumeAction action) {
            switch (action.Action) {
                case "Gs2AdReward:ConsumePointByUserId":
                    return ConsumePointByUserIdRequest.FromJson(JsonMapper.ToObject(action.Request));
            }
        ...
        public static Gs2Request ToRequest(Gs2.Core.Model.AcquireAction action) {
            switch (action.Action) {
                case "Gs2AdReward:AcquirePointByUserId":
                    return AcquirePointByUserIdRequest.FromJson(JsonMapper.ToObject(action.Request));
            }
            throw new ArgumentException($"unknown action {action.Action}");
        }
    }
