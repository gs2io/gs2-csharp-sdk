...
namespace Gs2.Gs2Showcase.Result
{
	...
	public class DescribeRandomShowcaseMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeRandomShowcaseMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeRandomShowcaseMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Showcase.Model.RandomShowcaseMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Showcase.Model.RandomShowcaseMaster.FromJson(v);
                }).ToArray())
