...
namespace Gs2.Gs2Showcase.Result
{
	...
	public class DescribeSalesItemMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeSalesItemMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeSalesItemMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Showcase.Model.SalesItemMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Showcase.Model.SalesItemMaster.FromJson(v);
                }).ToArray())
