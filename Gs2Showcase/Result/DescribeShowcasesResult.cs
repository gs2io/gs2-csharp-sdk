...
namespace Gs2.Gs2Showcase.Result
{
	...
	public class DescribeShowcasesResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeShowcasesResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeShowcasesResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Showcase.Model.Showcase[]{} : data["items"].Cast<JsonData>().Select(v => {
