...
namespace Gs2.Gs2Freeze.Model
{
	...
	public partial class Output : IComparable
	{
        ...
        private static System.Text.RegularExpressions.Regex _regionRegex = new System.Text.RegularExpressions.Regex(
                @"grn:gs2:(?<region>.+):(?<ownerId>.+):freeze:(?<stageName>.+):output:(?<outputName>.+)",
        ...
        public int CompareTo(object obj)
        {
            ...
            diff = ModelComparer.Compare(Text, other.Text);
            if (diff != 0)
            {
                return diff;
            }
        ...
        public void Validate() {
            {
                if (OutputId.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("output", "freeze.output.outputId.error.tooLong"),
                    });
                }
            }
            {
                if (Name.Length > 36) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("output", "freeze.output.name.error.tooLong"),
                    });
                }
            }
            {
                if (Text.Length > 1048576) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("output", "freeze.output.text.error.tooLong"),
                    });
                }
            }
            {
                if (CreatedAt < 0) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("output", "freeze.output.createdAt.error.invalid"),
                    });
                }
            ...
            }
            {
                if (Revision < 0) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("output", "freeze.output.revision.error.invalid"),
                    });
                }
