...
namespace Gs2.Gs2SerialKey.Result
{
	...
	public class DescribeCampaignModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeCampaignModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeCampaignModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2SerialKey.Model.CampaignModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2SerialKey.Model.CampaignModelMaster.FromJson(v);
                }).ToArray())
