...
namespace Gs2.Gs2SerialKey.Result
{
	...
	public class DescribeCampaignModelsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeCampaignModelsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeCampaignModelsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2SerialKey.Model.CampaignModel[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2SerialKey.Model.CampaignModel.FromJson(v);
                }).ToArray());
        }
