...
namespace Gs2.Gs2SerialKey.Result
{
	...
	public class DescribeIssueJobsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeIssueJobsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeIssueJobsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2SerialKey.Model.IssueJob[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2SerialKey.Model.IssueJob.FromJson(v);
                }).ToArray())
