...
namespace Gs2.Gs2Lottery.Result
{
	...
	public class DescribePrizeLimitsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribePrizeLimitsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribePrizeLimitsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Lottery.Model.PrizeLimit[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Lottery.Model.PrizeLimit.FromJson(v);
                }).ToArray())
