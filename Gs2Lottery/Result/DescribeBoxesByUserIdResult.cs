...
namespace Gs2.Gs2Lottery.Result
{
	...
	public class DescribeBoxesByUserIdResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeBoxesByUserIdResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeBoxesByUserIdResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Lottery.Model.BoxItems[]{} : data["items"].Cast<JsonData>().Select(v => {
