        ...
        public static Gs2Request ToRequest(Gs2.Core.Model.ConsumeAction action) {
            switch (action.Action) {
                case "Gs2Inbox:OpenMessageByUserId":
                    return OpenMessageByUserIdRequest.FromJson(JsonMapper.ToObject(action.Request));
                case "Gs2Inbox:DeleteMessageByUserId":
                    return DeleteMessageByUserIdRequest.FromJson(JsonMapper.ToObject(action.Request));
            }
        ...
        public static Gs2Request ToRequest(Gs2.Core.Model.AcquireAction action) {
            switch (action.Action) {
                case "Gs2Inbox:SendMessageByUserId":
                    return SendMessageByUserIdRequest.FromJson(JsonMapper.ToObject(action.Request));
            }
            throw new ArgumentException($"unknown action {action.Action}");
        }
    }
