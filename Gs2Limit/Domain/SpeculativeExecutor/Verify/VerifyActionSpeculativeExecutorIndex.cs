...
/* diff +++ start */
        ...
        public static async Task<Func<object>> ExecuteInverseAsync(
            ...
            if (VerifyCounterByUserIdSpeculativeExecutor.Action() != verifyAction.Action) {
                return null;
            }
            var request = VerifyCounterByUserIdRequest.FromJson(
                JsonMapper.ToObject(verifyAction.Request)
            );
            if (rate != 1) {
                request = request.Rate(rate);
            }
            return await VerifyCounterByUserIdSpeculativeExecutor
...
/* diff +++ end */
        ...
        public static async Task<Func<object>> ExecuteAsync(
            ...
            verifyAction.Action = verifyAction.Action.Replace("{userId}", accessToken.UserId);
            if (VerifyCounterByUserIdSpeculativeExecutor.Action() == verifyAction.Action) {
                var request = VerifyCounterByUserIdRequest.FromJson(JsonMapper.ToObject(verifyAction.Request));
                if (rate != 1) {
                    request = request.Rate(rate);
                }
