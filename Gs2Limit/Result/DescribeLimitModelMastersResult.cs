...
namespace Gs2.Gs2Limit.Result
{
	...
	public class DescribeLimitModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeLimitModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeLimitModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Limit.Model.LimitModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Limit.Model.LimitModelMaster.FromJson(v);
                }).ToArray())
