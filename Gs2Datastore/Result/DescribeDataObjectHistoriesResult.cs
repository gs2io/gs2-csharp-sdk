...
namespace Gs2.Gs2Datastore.Result
{
	...
	public class DescribeDataObjectHistoriesResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeDataObjectHistoriesResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeDataObjectHistoriesResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null ? new Gs2.Gs2Datastore.Model.DataObjectHistory[]{} : data["items"].Cast<JsonData>().Select(v => {
