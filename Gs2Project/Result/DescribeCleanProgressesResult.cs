...
namespace Gs2.Gs2Project.Result
{
	...
	public class DescribeCleanProgressesResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeCleanProgressesResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeCleanProgressesResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Project.Model.CleanProgress[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Project.Model.CleanProgress.FromJson(v);
                }).ToArray())
