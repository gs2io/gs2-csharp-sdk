...
namespace Gs2.Gs2Project.Result
{
	...
	public class DescribeBillingsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeBillingsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeBillingsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Project.Model.Billing[]{} : data["items"].Cast<JsonData>().Select(v => {
