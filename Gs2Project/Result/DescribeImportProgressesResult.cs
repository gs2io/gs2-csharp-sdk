...
namespace Gs2.Gs2Project.Result
{
	...
	public class DescribeImportProgressesResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeImportProgressesResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeImportProgressesResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Project.Model.ImportProgress[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Project.Model.ImportProgress.FromJson(v);
                }).ToArray())
