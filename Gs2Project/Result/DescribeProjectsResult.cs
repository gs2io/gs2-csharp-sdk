...
namespace Gs2.Gs2Project.Result
{
	...
	public class DescribeProjectsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeProjectsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeProjectsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Project.Model.Project[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Project.Model.Project.FromJson(v);
                }).ToArray())
