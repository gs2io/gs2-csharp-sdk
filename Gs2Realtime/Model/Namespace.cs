...
namespace Gs2.Gs2Realtime.Model
{
	...
	public partial class Namespace : IComparable
	{
        ...
        private static System.Text.RegularExpressions.Regex _regionRegex = new System.Text.RegularExpressions.Regex(
                @"grn:gs2:(?<region>.+):(?<ownerId>.+):realtime:(?<namespaceName>.+)",
        ...
        public int CompareTo(object obj)
        {
            ...
            diff = ModelComparer.Compare(ServerType, other.ServerType);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(ServerSpec, other.ServerSpec);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(CreateNotification, other.CreateNotification);
            if (diff != 0)
            {
                return diff;
            }
        ...
        public void Validate() {
            {
                if (NamespaceId.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("namespace", "realtime.namespace.namespaceId.error.tooLong"),
                    });
                }
            }
            {
                if (Name.Length > 128) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("namespace", "realtime.namespace.name.error.tooLong"),
                    });
                }
            }
            {
                if (Description.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("namespace", "realtime.namespace.description.error.tooLong"),
                    });
                }
            }
            {
            }
            {
                switch (ServerType) {
                    ...
                    default:
                        throw new Gs2.Core.Exception.BadRequestException(new [] {
                            new RequestError("namespace", "realtime.namespace.serverType.error.invalid"),
                        });
                }
            }
            {
                switch (ServerSpec) {
                    ...
                    default:
                        throw new Gs2.Core.Exception.BadRequestException(new [] {
                            new RequestError("namespace", "realtime.namespace.serverSpec.error.invalid"),
                        });
                }
            }
            {
            ...
            }
            {
                if (CreatedAt < 0) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("namespace", "realtime.namespace.createdAt.error.invalid"),
                    });
                }
            ...
            }
            {
                if (UpdatedAt < 0) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("namespace", "realtime.namespace.updatedAt.error.invalid"),
                    });
                }
            ...
            }
            {
                if (Revision < 0) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("namespace", "realtime.namespace.revision.error.invalid"),
                    });
                }
        ...
        public object Clone() {
            return new Namespace {
                ...
                LogSetting = LogSetting?.Clone() as Gs2.Gs2Realtime.Model.LogSetting,
                CreatedAt = CreatedAt,
                UpdatedAt = UpdatedAt,
                Revision = Revision,
            };
