...
namespace Gs2.Gs2Account.Result
{
	...
	public class DescribeAccountsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeAccountsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeAccountsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Account.Model.Account[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Account.Model.Account.FromJson(v);
                }).ToArray())
