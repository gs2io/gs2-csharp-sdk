...
namespace Gs2.Gs2Ranking2.Model
{
	...
	[Preserve]
#endif
	public partial class GlobalRankingData : IComparable
	{
        public string GlobalRankingDataId { set; get; }
        ...
        private static System.Text.RegularExpressions.Regex _regionRegex = new System.Text.RegularExpressions.Regex(
                @"grn:gs2:(?<region>.+):(?<ownerId>.+):ranking2:(?<namespaceName>.+):global:(?<rankingName>.+):ranking:global:(?<season>.+):user:(?<scorerUserId>.+):score",
        ...
        public int CompareTo(object obj)
        {
            ...
            if (ReferenceEquals(other, null))
            {
                ...
                throw new ArgumentException("Object must be of type GlobalRankingData.", nameof(obj));
            }
            var diff = 0;
            diff = ModelComparer.Compare(GlobalRankingDataId, other.GlobalRankingDataId);
            if (diff != 0)
            {
                return diff;
            }
        ...
        public void Validate() {
            ...
            }
            {
                if (InvertUpdatedAt < 0) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("globalRankingData", "ranking2.globalRankingData.invertUpdatedAt.error.invalid"),
                    });
                }
