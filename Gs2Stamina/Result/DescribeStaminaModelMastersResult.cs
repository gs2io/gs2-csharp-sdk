...
namespace Gs2.Gs2Stamina.Result
{
	...
	public class DescribeStaminaModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeStaminaModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeStaminaModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Stamina.Model.StaminaModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Stamina.Model.StaminaModelMaster.FromJson(v);
                }).ToArray())
