...
{
    public static class VerifyStaminaRecoverIntervalMinutesByUserIdSpeculativeExecutor {
/* diff +++ start */
            ...
            IPreparedSpeculativeVerification
        {
            ...
            private readonly VerifyStaminaRecoverIntervalMinutesByUserIdRequest _request;
...
/* diff +++ start */
        ...
        private static bool IsUsable(
            ...
            return item != null &&
                   ...
                   item.RecoverIntervalMinutes.HasValue;
        }
        ...
        public static async Task<Func<object>> ExecuteInverseAsync(
            ...
            var inverse = request == null ? null :
                VerifyStaminaRecoverIntervalMinutesByUserIdRequest.FromJson(request.ToJson());
...
/* diff +++ start */
            ...
            var prepared = VerifyStaminaRecoverIntervalMinutesByUserIdRequest.FromJson(
                request.ToJson()
            );
            if (prepared.UserId == "#{userId}") prepared.UserId = token.UserId;
            if (prepared.UserId != token.UserId ||
                !prepared.Value.HasValue ||
                !IsSupportedVerifyType(prepared.VerifyType)) return null;
            ...
            if (!cached.Item2 ||
                !IsUsable(item, prepared, token.UserId, expectedId)) return null;
