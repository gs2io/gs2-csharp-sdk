...
{
    public static class DecreaseMaxValueByUserIdSpeculativeExecutor {
...
/* diff +++ start */
            ...
            var prepared = DecreaseMaxValueByUserIdRequest.FromJson(
                request.ToJson()
            );
            if (prepared.UserId == "#{userId}") prepared.UserId = token.UserId;
            if (prepared.UserId != token.UserId ||
                !prepared.DecreaseValue.HasValue) return null;
            ...
            var decreaseValue = prepared.DecreaseValue.Value;
            return new StaminaMutationSpeculativeCommit(
                ...
                (current, cap) => {
                    current.MaxValue = StaminaMutationSpeculativeCommit.Clamp(
                        (long)current.MaxValue.Value - decreaseValue, cap
                    );
                    return true;
                }
            ).Invoke;
