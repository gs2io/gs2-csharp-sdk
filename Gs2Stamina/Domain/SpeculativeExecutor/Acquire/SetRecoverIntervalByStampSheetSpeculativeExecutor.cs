...
{
    public static class SetRecoverIntervalByUserIdSpeculativeExecutor {
...
/* diff +++ start */
            ...
            var prepared = SetRecoverIntervalByUserIdRequest.FromJson(
                request.ToJson()
            );
            if (prepared.UserId == "#{userId}") prepared.UserId = token.UserId;
            if (prepared.UserId != token.UserId ||
                !prepared.RecoverIntervalMinutes.HasValue) return null;
            ...
            var recoverIntervalMinutes = prepared.RecoverIntervalMinutes.Value;
            return new StaminaMutationSpeculativeCommit(
                ...
                (current, _) => {
                    current.RecoverIntervalMinutes = recoverIntervalMinutes;
                    return true;
                }
            ).Invoke;
/* diff +++ end */
