...
{
    public static class SetMaxValueByUserIdSpeculativeExecutor {
...
/* diff +++ start */
            ...
            var prepared = SetMaxValueByUserIdRequest.FromJson(request.ToJson());
            if (prepared.UserId == "#{userId}") prepared.UserId = token.UserId;
            if (prepared.UserId != token.UserId ||
                !prepared.MaxValue.HasValue) return null;
            ...
            var maxValue = prepared.MaxValue.Value;
            return new StaminaMutationSpeculativeCommit(
                ...
                (current, cap) => {
                    current.MaxValue =
                        StaminaMutationSpeculativeCommit.Clamp(maxValue, cap);
                    return true;
                }
            ).Invoke;
/* diff +++ end */
