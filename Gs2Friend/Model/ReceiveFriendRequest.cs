...
namespace Gs2.Gs2Friend.Model
{
	...
	public partial class ReceiveFriendRequest : IComparable
	{
    	...
    	[Preserve]
#endif
        public static ReceiveFriendRequest FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
        ...
        {
            var other = obj as ReceiveFriendRequest;
            if (ReferenceEquals(other, null))
            {
                ...
                throw new ArgumentException("Object must be of type ReceiveFriendRequest.", nameof(obj));
            }
            var diff = 0;
        ...
        public void Validate() {
            {
                if (UserId.Length > 128) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("receiveFriendRequest", "friend.receiveFriendRequest.userId.error.tooLong"),
                    });
                }
            }
            {
                if (TargetUserId.Length > 128) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("receiveFriendRequest", "friend.receiveFriendRequest.targetUserId.error.tooLong"),
                    });
                }
            }
            {
                if (PublicProfile.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("receiveFriendRequest", "friend.receiveFriendRequest.publicProfile.error.tooLong"),
                    });
                }
            }
        }
