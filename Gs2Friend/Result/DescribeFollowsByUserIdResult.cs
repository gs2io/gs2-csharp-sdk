...
namespace Gs2.Gs2Friend.Result
{
	...
	public class DescribeFollowsByUserIdResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeFollowsByUserIdResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeFollowsByUserIdResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Friend.Model.FollowUser[]{} : data["items"].Cast<JsonData>().Select(v => {
