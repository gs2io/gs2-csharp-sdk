...
namespace Gs2.Gs2SkillTree.Model.Cache
{
    public static partial class NodeModelExt
    {
        public static void PutCache(
            this DescribeNodeModelsResult self,
            CacheDatabase cache,
            string userId,
            DescribeNodeModelsRequest request
        ) {
            foreach (var item in self.Items ?? Array.Empty<NodeModel>())
            {
                item.PutCache(
                    cache,
...
#if UNITY_2017_1_OR_NEWER
        public static IFuture<DescribeNodeModelsResult> InvokeFuture(
            this DescribeNodeModelsRequest request,
            CacheDatabase cache,
            string userId,
            Func<IFuture<DescribeNodeModelsResult>> invokeImpl
        )
        {
            IEnumerator Impl(IFuture<DescribeNodeModelsResult> self)
            {
            ...
            return new Gs2InlineFuture<DescribeNodeModelsResult>(Impl);
        }
#endif
