...
namespace Gs2.Gs2Matchmaking.Model.Cache
{
    public static class Gs2Matchmaking
    {
        public static string PutUserData(
            ...
            switch (kind) {
                case "joinedSeasonGathering":
                    return Gs2.Gs2Matchmaking.Model.JoinedSeasonGathering.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
                case "rating":
                    return Gs2.Gs2Matchmaking.Model.Rating.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
            }
            return null;
        }
        ...
        public static bool SetListCached(
            ...
            switch (kind) {
                case "joinedSeasonGathering":
                    cache.SetListCached<Gs2.Gs2Matchmaking.Model.JoinedSeasonGathering>(parentKey);
                    return true;
                case "rating":
                    cache.SetListCached<Gs2.Gs2Matchmaking.Model.Rating>(parentKey);
                    return true;
            }
            return false;
        }
