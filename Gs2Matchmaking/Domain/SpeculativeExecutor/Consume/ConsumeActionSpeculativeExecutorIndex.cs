    ...
    public static class ConsumeActionSpeculativeExecutorIndex
    {
        ...
        public static Gs2Future<Func<object>> ExecuteFuture(
            ...
            IEnumerator Impl(Gs2Future<Func<object>> result) {
                if (VerifyIncludeParticipantByUserIdSpeculativeExecutor.Action() == consumeAction.Action) {
                    var request = VerifyIncludeParticipantByUserIdRequest.FromJson(JsonMapper.ToObject(consumeAction.Request));
                    if (rate != 1) {
                        request = request.Rate(rate);
                    }
                    var future = VerifyIncludeParticipantByUserIdSpeculativeExecutor.ExecuteFuture(
                        domain,
                        accessToken,
                        request
                    );
