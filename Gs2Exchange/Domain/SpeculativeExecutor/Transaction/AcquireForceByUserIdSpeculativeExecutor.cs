...
namespace Gs2.Gs2Exchange.Domain.Transaction.SpeculativeExecutor
{
    public static class AcquireForceByUserIdSpeculativeExecutor {
        ...
        public static async Task<Func<object>> ExecuteAsync(
            ...
            var prepared = AcquireForceByUserIdRequest.FromJson(request?.ToJson());
            ...
            foreach (var config in item.Config ?? Array.Empty<Config>()) {
                if (config != null &&
                    configs.All(v => v.Key != config.Key)) {
                    configs.Add(config);
                }
            }
            ...
            try {
                acquireActions = (rateModel?.AcquireActions ??
                    ...
                    .Where(v => v != null &&
                        v.Action != Gs2.Gs2Exchange.Domain.SpeculativeExecutor
                            .AcquireForceByUserIdSpeculativeExecutor.Action())
                    .Select(v => {
