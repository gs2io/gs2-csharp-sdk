...
namespace Gs2.Gs2Version.Result
{
	...
	public class DescribeVersionModelsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeVersionModelsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeVersionModelsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Version.Model.VersionModel[]{} : data["items"].Cast<JsonData>().Select(v => {
