	...
	public partial class AcquireActionRate : IComparable
	{
        ...
        public int CompareTo(object obj)
        {
            ...
            if (ReferenceEquals(other, null))
            {
                ...
                throw new ArgumentException("Object must be of type AcquireActionRate.", nameof(obj));
            }
            var diff = 0;
            ...
            diff = ModelComparer.CompareArray(Rates, other.Rates);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.CompareArray(BigRates, other.BigRates);
            if (diff != 0)
            {
                return diff;
            }
        ...
        public void Validate() {
            {
                if (Name.Length > 128) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("acquireActionRate", "grade.acquireActionRate.name.error.tooLong"),
                    });
                }
            }
            {
                switch (Mode) {
                    ...
                    default:
                        throw new Gs2.Core.Exception.BadRequestException(new [] {
                            new RequestError("acquireActionRate", "grade.acquireActionRate.mode.error.invalid"),
                        });
                }
            }
            if (Mode == "double") {
                if (Rates.Length < 1) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("acquireActionRate", "grade.acquireActionRate.rates.error.tooFew"),
                    });
                }
                if (Rates.Length > 1000) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("acquireActionRate", "grade.acquireActionRate.rates.error.tooMany"),
                    });
                }
            }
            if (Mode == "big") {
                if (BigRates.Length < 1) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("acquireActionRate", "grade.acquireActionRate.bigRates.error.tooFew"),
                    });
                }
                if (BigRates.Length > 1000) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("acquireActionRate", "grade.acquireActionRate.bigRates.error.tooMany"),
                    });
                }
            }
        }
