	...
	public class DescribeNamespacesResult : IResult
	{
        ...
        public static DescribeNamespacesResult FromJson(JsonData data)
        {
            ...
            return new DescribeNamespacesResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Grade.Model.Namespace[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Grade.Model.Namespace.FromJson(v);
                }).ToArray())
