...
namespace Gs2.Gs2Grade.Result
{
	...
	public class DescribeGradeModelsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeGradeModelsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeGradeModelsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Grade.Model.GradeModel[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Grade.Model.GradeModel.FromJson(v);
                }).ToArray());
        }
