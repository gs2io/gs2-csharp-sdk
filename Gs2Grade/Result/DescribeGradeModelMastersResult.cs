...
namespace Gs2.Gs2Grade.Result
{
	...
	public class DescribeGradeModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeGradeModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeGradeModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Grade.Model.GradeModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Grade.Model.GradeModelMaster.FromJson(v);
                }).ToArray())
