...
/* diff +++ start */
        ...
        public static async Task<Func<object>> ExecuteInverseAsync(
            ...
            if (VerifyGradeByUserIdSpeculativeExecutor.Action() == verifyAction.Action) {
                var request = VerifyGradeByUserIdRequest.FromJson(JsonMapper.ToObject(verifyAction.Request));
                if (rate != 1) {
                    request = request.Rate(rate);
                }
                return await VerifyGradeByUserIdSpeculativeExecutor.ExecuteInverseAsync(
                    domain,
                    accessToken,
                    request
                );
            }
            if (VerifyGradeUpMaterialByUserIdSpeculativeExecutor.Action() == verifyAction.Action) {
                var request = VerifyGradeUpMaterialByUserIdRequest.FromJson(JsonMapper.ToObject(verifyAction.Request));
                if (rate != 1) {
                    request = request.Rate(rate);
                }
                return await VerifyGradeUpMaterialByUserIdSpeculativeExecutor.ExecuteInverseAsync(
                    domain,
                    accessToken,
                    request
                );
