...
namespace Gs2.Gs2Identifier.Result
{
	...
	public class DescribeIdentifiersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeIdentifiersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeIdentifiersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Identifier.Model.Identifier[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Identifier.Model.Identifier.FromJson(v);
                }).ToArray())
