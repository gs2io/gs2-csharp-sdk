...
namespace Gs2.Gs2Identifier.Result
{
	...
	public class DescribePasswordsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribePasswordsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribePasswordsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Identifier.Model.Password[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Identifier.Model.Password.FromJson(v);
                }).ToArray())
