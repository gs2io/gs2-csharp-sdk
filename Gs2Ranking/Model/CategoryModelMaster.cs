	...
	public partial class CategoryModelMaster : IComparable
	{
        ...
        public static CategoryModelMaster FromJson(JsonData data)
        {
            ...
            return new CategoryModelMaster()
                ...
                .WithCalculateFixedTimingHour(!data.Keys.Contains("calculateFixedTimingHour") || data["calculateFixedTimingHour"] == null ? null : Gs2.Core.Util.JsonValue.ToNullableInt(data["calculateFixedTimingHour"].ToString()))
                .WithCalculateFixedTimingMinute(!data.Keys.Contains("calculateFixedTimingMinute") || data["calculateFixedTimingMinute"] == null ? null : Gs2.Core.Util.JsonValue.ToNullableInt(data["calculateFixedTimingMinute"].ToString()))
        ...
        public int CompareTo(object obj)
        {
            ...
            diff = ModelComparer.Compare(GlobalRankingSetting, other.GlobalRankingSetting);
            if (diff != 0)
            {
                return diff;
            }
            ...
            diff = ModelComparer.Compare(CalculateFixedTimingHour, other.CalculateFixedTimingHour);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(CalculateFixedTimingMinute, other.CalculateFixedTimingMinute);
            if (diff != 0)
            {
                return diff;
            }
