...
namespace Gs2.Gs2Ranking.Model.Cache
{
    public static class Gs2Ranking
    {
        ...
        public static string PutUserData(
            ...
            switch (kind) {
                case "score":
                    return Gs2.Gs2Ranking.Model.Score.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
                case "subscribeUser":
                    return Gs2.Gs2Ranking.Model.SubscribeUser.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
            }
            return null;
        }
        ...
        public static bool SetListCached(
            ...
            switch (kind) {
                case "score":
                    cache.SetListCached<Gs2.Gs2Ranking.Model.Score>(parentKey);
                    return true;
                case "subscribeUser":
                    cache.SetListCached<Gs2.Gs2Ranking.Model.SubscribeUser>(parentKey);
                    return true;
            }
            return false;
        }
