...
namespace Gs2.Gs2Ranking.Result
{
	...
	public class DescribeScoresByUserIdResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeScoresByUserIdResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeScoresByUserIdResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Ranking.Model.Score[]{} : data["items"].Cast<JsonData>().Select(v => {
