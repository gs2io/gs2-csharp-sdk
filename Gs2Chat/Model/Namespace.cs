	...
	public partial class Namespace : IComparable
	{
        ...
        public static Namespace FromJson(JsonData data)
        {
            ...
            return new Namespace()
                ...
                .WithAllowCreateRoom(!data.Keys.Contains("allowCreateRoom") || data["allowCreateRoom"] == null ? null : Gs2.Core.Util.JsonValue.ToNullableBool(data["allowCreateRoom"].ToString()))
                .WithMessageLifeTimeDays(!data.Keys.Contains("messageLifeTimeDays") || data["messageLifeTimeDays"] == null ? null : Gs2.Core.Util.JsonValue.ToNullableInt(data["messageLifeTimeDays"].ToString()))
        ...
        public int CompareTo(object obj)
        {
            ...
            diff = ModelComparer.Compare(AllowCreateRoom, other.AllowCreateRoom);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(MessageLifeTimeDays, other.MessageLifeTimeDays);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(PostMessageScript, other.PostMessageScript);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(CreateRoomScript, other.CreateRoomScript);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(DeleteRoomScript, other.DeleteRoomScript);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(SubscribeRoomScript, other.SubscribeRoomScript);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(UnsubscribeRoomScript, other.UnsubscribeRoomScript);
            if (diff != 0)
            {
                return diff;
            }
            diff = ModelComparer.Compare(PostNotification, other.PostNotification);
            if (diff != 0)
            {
                return diff;
            }
