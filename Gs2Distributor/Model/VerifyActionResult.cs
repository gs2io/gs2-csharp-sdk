        ...
        public void Validate() {
            {
                if (Action.Length > 128) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("verifyActionResult", "distributor.verifyActionResult.action.error.tooLong"),
                    });
                }
            }
            {
            ...
            }
            {
                if (StatusCode < 0) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("verifyActionResult", "distributor.verifyActionResult.statusCode.error.invalid"),
                    });
                }
                if (StatusCode > 999) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
            ...
            }
            {
                if (VerifyResult.Length > 1048576) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("verifyActionResult", "distributor.verifyActionResult.verifyResult.error.tooLong"),
                    });
                }
            }
        }
