...
namespace Gs2.Gs2Distributor.Model
{
	...
	[Preserve]
#endif
	public partial class ConsumeActionResult : IComparable
	{
        public string Action { set; get; }
        public string ConsumeRequest { set; get; }
        public int? StatusCode { set; get; }
        public string ConsumeResult { set; get; }
        ...
        public void Validate() {
            {
                if (Action.Length > 128) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("consumeActionResult", "distributor.consumeActionResult.action.error.tooLong"),
                    });
                }
            }
            {
                if (ConsumeRequest.Length > 524288) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("consumeActionResult", "distributor.consumeActionResult.consumeRequest.error.tooLong"),
                    });
                }
            }
            {
                if (StatusCode < 0) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("consumeActionResult", "distributor.consumeActionResult.statusCode.error.invalid"),
                    });
                }
                if (StatusCode > 999) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
            ...
            }
            {
                if (ConsumeResult.Length > 1048576) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("consumeActionResult", "distributor.consumeActionResult.consumeResult.error.tooLong"),
                    });
                }
            }
        }
