...
namespace Gs2.Gs2Script.Result
{
	...
	public class DescribeScriptsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeScriptsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeScriptsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Script.Model.Script[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Script.Model.Script.FromJson(v);
                }).ToArray())
