...
namespace Gs2.Gs2Formation.Result
{
	...
	public class DescribePropertyFormModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribePropertyFormModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribePropertyFormModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Formation.Model.PropertyFormModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Formation.Model.PropertyFormModelMaster.FromJson(v);
                }).ToArray())
