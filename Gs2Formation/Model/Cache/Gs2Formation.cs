...
namespace Gs2.Gs2Formation.Model.Cache
{
    public static class Gs2Formation
    {
        ...
        public static string PutUserData(
            ...
            switch (kind) {
                case "form":
                    return Gs2.Gs2Formation.Model.Form.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
                case "mold":
                    return Gs2.Gs2Formation.Model.Mold.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
                case "propertyForm":
                    return Gs2.Gs2Formation.Model.PropertyForm.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
            }
            return null;
        }
        ...
        public static bool SetListCached(
            ...
            switch (kind) {
                case "form":
                    cache.SetListCached<Gs2.Gs2Formation.Model.Form>(parentKey);
                    return true;
                case "mold":
                    cache.SetListCached<Gs2.Gs2Formation.Model.Mold>(parentKey);
                    return true;
                case "propertyForm":
                    cache.SetListCached<Gs2.Gs2Formation.Model.PropertyForm>(parentKey);
                    return true;
            }
            return false;
        }
