...
namespace Gs2.Gs2Enchant.Result
{
	...
	public class DescribeBalanceParameterModelsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeBalanceParameterModelsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeBalanceParameterModelsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Enchant.Model.BalanceParameterModel[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Enchant.Model.BalanceParameterModel.FromJson(v);
                }).ToArray());
        }
