...
{
/* diff +++ start */
    internal sealed class RarityParameterStatusSpeculativeCommit :
        IComposableSpeculativeCommit
    {
        ...
        private readonly RarityParameterStatus _preparedItem;
        private readonly Func<RarityParameterStatus, RarityParameterStatus>
            _transform;

        internal RarityParameterStatusSpeculativeCommit(
            CacheDatabase cache,
            ...
            RarityParameterStatus preparedItem,
            Func<RarityParameterStatus, RarityParameterStatus> transform
        ) {
            _cache = cache;
        ...
        private bool IsExpectedStatus(RarityParameterStatus item) {
            return item != null &&
        ...
        public bool TryCompose(
            ...
            try {
                if (hasCurrent) {
                    if (current is not RarityParameterStatus currentStatus ||
                        !IsExpectedStatus(currentStatus)) {
                        next = null;
                        return false;
                    }
                    next = _transform(currentStatus);
                    return IsExpectedStatus(next as RarityParameterStatus);
                }
                var (cachedItem, find) = ((RarityParameterStatus)null).GetCache(
                    _cache,
                    _namespaceName,
                    _userId,
                    _parameterName,
        ...
        public object Commit(object value) {
            if (value is RarityParameterStatus item &&
...
/* diff +++ end */
    public static class SetRarityParameterStatusByUserIdSpeculativeExecutor {
...
/* diff +++ start */
        ...
        private static SetRarityParameterStatusByUserIdRequest Snapshot(
            ...
            var values = parameterValues
                ...
                .ToArray();
            if (parameterValues.Length > 0 && values.Length == 0) {
                return null;
            }
...
/* diff +++ start */
            ...
            RarityParameterStatus Transform(RarityParameterStatus source) {
...
/* diff --- start
 ...
 diff --- end */
/* diff +++ start */
            var cached = ((RarityParameterStatus)null).GetCache(
                domain.Cache,
                namespaceName,
                userId,
                parameterName,
...
/* diff +++ start */
            ...
            var source = item ?? new RarityParameterStatus()
                .WithRarityParameterStatusId(expectedStatusId)
                .WithUserId(userId)
                .WithParameterName(parameterName)
                .WithPropertyId(propertyId)
                .WithParameterValues(Array.Empty<RarityParameterValue>())
                .WithCreatedAt(currentTimeMillis)
                .WithUpdatedAt(currentTimeMillis)
                .WithRevision(0);
            var preparedItem = Transform(source);
            var speculativeCommit = new RarityParameterStatusSpeculativeCommit(
                domain.Cache,
                namespaceName,
                userId,
                parameterName,
