...
namespace Gs2.Gs2Enchant
{
	public class Gs2EnchantWebSocketClient : AbstractGs2Client
	{
        ...
        public class GetRarityParameterModelTask : Gs2WebSocketSessionTask<Request.GetRarityParameterModelRequest, Result.GetRarityParameterModelResult>
        {
	        public GetRarityParameterModelTask(IGs2Session session, Request.GetRarityParameterModelRequest request) : base(session, request)
	        {
	        }

            protected override IGs2SessionRequest CreateRequest(Request.GetRarityParameterModelRequest request)
            {
                var stringBuilder = new StringBuilder();
                var jsonWriter = new JsonWriter(stringBuilder);
