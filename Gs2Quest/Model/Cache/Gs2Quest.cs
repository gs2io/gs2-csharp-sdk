...
namespace Gs2.Gs2Quest.Model.Cache
{
    public static class Gs2Quest
    {
        ...
        public static string PutUserData(
            ...
            switch (kind) {
                case "completedQuestList":
                    return Gs2.Gs2Quest.Model.CompletedQuestList.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
                case "progress":
                    return Gs2.Gs2Quest.Model.Progress.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
            }
            return null;
        }
        ...
        public static bool SetListCached(
            ...
            switch (kind) {
                case "completedQuestList":
                    cache.SetListCached<Gs2.Gs2Quest.Model.CompletedQuestList>(parentKey);
                    return true;
                case "progress":
                    cache.SetListCached<Gs2.Gs2Quest.Model.Progress>(parentKey);
                    return true;
            }
            return false;
        }
