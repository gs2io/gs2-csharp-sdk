...
namespace Gs2.Gs2Log.Domain.Iterator
{
    ...
    #endif
    {
        ...
        public string LabelNamePrefix { get; }
        private string _pageToken;
        private bool _isCacheChecked;
        private bool _last;
        private Gs2.Gs2Log.Model.Label[] _result;

        public static int? fetchSize;

        public DescribeLabelValuesIterator(
            ...
            this._last = false;
            this._result = new Gs2.Gs2Log.Model.Label[]{};
        }
...
/* diff --- start
 ...
 diff --- end */
                ...
                this._result = list
/* diff --- start
                    .Where(item => this.MetricName == null || item.MetricName == this.MetricName)
                    .Where(item => this.LabelNamePrefix == null || item.LabelName.StartsWith(this.LabelNamePrefix))
 diff --- end */
                    .Where(item => this.LabelNamePrefix == null || item.Key.StartsWith(this.LabelNamePrefix)) /* diff +++ */
                    .ToArray();
                this._pageToken = null;
                this._last = true;
            } else {

                var request = new Gs2.Gs2Log.Request.DescribeLabelValuesRequest()
                    .WithContextStack(this._gs2.DefaultContextStack)
                    .WithNamespaceName(this.NamespaceName)
                    .WithMetricName(this.MetricName)
                    .WithPageToken(this._pageToken)
                    .WithLimit(fetchSize);
                var r = await this._client.DescribeLabelValuesAsync(
                    request
                );
...
/* diff --- start
 ...
 diff --- end */
        ...
        protected override System.Collections.IEnumerator Next(
            Action<AsyncResult<Gs2.Gs2Log.Model.Label>> callback
        )
        {
            if (this._result.Length == 0 && !this._last) {
                ...
                if (future.Error != null)
                {
                    Current = null;
                    Error = future.Error;
                    callback.Invoke(new AsyncResult<Gs2.Gs2Log.Model.Label>(
                        Current,
                        Error
                    ));
                    yield break;
        ...
        ) => UniTaskAsyncEnumerable.Create<Gs2.Gs2Log.Model.Label>(async (writer, token) =>
        #else
        public async IAsyncEnumerator<Gs2.Gs2Log.Model.Label> GetAsyncEnumerator(
            CancellationToken cancellationToken = new CancellationToken()
        )
        #endif
        {
