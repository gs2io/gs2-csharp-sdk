...
/* diff --- start
            var request = new Gs2.Gs2Log.Request.CountExecuteStampSheetLogRequest()
...
/* diff --- start
 ...
 diff --- end */
        ...
        ) => UniTaskAsyncEnumerable.Create<Gs2.Gs2Log.Model.ExecuteStampSheetLogCount>(async (writer, token) =>
        #else
        ...
        {
/* diff --- start
            using (await this._gs2.Cache.GetLockObject<Gs2.Gs2Log.Model.ExecuteStampSheetLogCount>(
                    (null as Gs2.Gs2Log.Model.ExecuteStampSheetLogCount).CacheParentKey(
                        NamespaceName,
                        null
                   ),
                   "ListExecuteStampSheetLogCount"
               ).LockAsync()) {
                while(this._hasNext()) {
