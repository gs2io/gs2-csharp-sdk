...
namespace Gs2.Gs2Log.Result
{
	...
	public class QueryAccessLogWithTelemetryResult : IResult
	{
        ...
        public QueryAccessLogWithTelemetryResult WithMetadata(ResultMetadata metadata) {
    	...
    	[Preserve]
#endif
        public static QueryAccessLogWithTelemetryResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new QueryAccessLogWithTelemetryResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? null : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Log.Model.AccessLogWithTelemetry.FromJson(v);
                }).ToArray())
