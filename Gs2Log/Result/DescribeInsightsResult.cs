...
namespace Gs2.Gs2Log.Result
{
	...
	public class DescribeInsightsResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeInsightsResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeInsightsResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Log.Model.Insight[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Log.Model.Insight.FromJson(v);
                }).ToArray())
