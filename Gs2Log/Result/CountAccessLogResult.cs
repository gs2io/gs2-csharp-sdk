...
#endif
	[System.Serializable]
	public class CountAccessLogResult : IResult
	{
        public Gs2.Gs2Log.Model.AccessLogCount[] Items { set; get; }
        public string NextPageToken { set; get; }
        public long? TotalCount { set; get; }
        public long? ScanSize { set; get; }
        public ResultMetadata Metadata { set; get; }

        public CountAccessLogResult WithItems(Gs2.Gs2Log.Model.AccessLogCount[] items) {
            this.Items = items;
            return this;
        }
        ...
        public CountAccessLogResult WithMetadata(ResultMetadata metadata) {
    	...
    	[Preserve]
#endif
        public static CountAccessLogResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new CountAccessLogResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? null : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Log.Model.AccessLogCount.FromJson(v);
                }).ToArray())
