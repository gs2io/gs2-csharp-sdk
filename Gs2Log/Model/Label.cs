...
namespace Gs2.Gs2Log.Model
{
	...
	public partial class Label : IComparable
	{
    	...
    	[Preserve]
#endif
        public static Label FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
        ...
        public int CompareTo(object obj)
        {
            ...
            if (ReferenceEquals(other, null))
            {
                ...
                throw new ArgumentException("Object must be of type Label.", nameof(obj));
            }
            var diff = 0;
        ...
        public void Validate() {
            {
                if (Key.Length > 128) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("label", "log.label.key.error.tooLong"),
                    });
                }
            }
            {
                if (Value.Length > 256) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("label", "log.label.value.error.tooLong"),
                    });
                }
            }
        }
