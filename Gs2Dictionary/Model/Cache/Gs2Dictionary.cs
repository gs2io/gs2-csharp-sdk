...
namespace Gs2.Gs2Dictionary.Model.Cache
{
    public static class Gs2Dictionary
    {
        ...
        public static string PutUserData(
            ...
            switch (kind) {
                case "entry":
                    return Gs2.Gs2Dictionary.Model.Entry.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
                case "like":
                    return Gs2.Gs2Dictionary.Model.Like.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
            }
            return null;
        }
        ...
        public static bool SetListCached(
            ...
            switch (kind) {
                case "entry":
                    cache.SetListCached<Gs2.Gs2Dictionary.Model.Entry>(parentKey);
                    return true;
                case "like":
                    cache.SetListCached<Gs2.Gs2Dictionary.Model.Like>(parentKey);
                    return true;
            }
            return false;
        }
