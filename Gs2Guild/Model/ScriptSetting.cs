        ...
        public void Validate() {
            {
                if (TriggerScriptId.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("scriptSetting", "guild.scriptSetting.triggerScriptId.error.tooLong"),
                    });
                }
            }
            {
                switch (DoneTriggerTargetType) {
                    ...
                    default:
                        throw new Gs2.Core.Exception.BadRequestException(new [] {
                            new RequestError("scriptSetting", "guild.scriptSetting.doneTriggerTargetType.error.invalid"),
                        });
                }
            }
            {
                if (DoneTriggerScriptId.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("scriptSetting", "guild.scriptSetting.doneTriggerScriptId.error.tooLong"),
                    });
                }
            }
            {
                if (DoneTriggerQueueNamespaceId.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("scriptSetting", "guild.scriptSetting.doneTriggerQueueNamespaceId.error.tooLong"),
                    });
                }
            }
        }
