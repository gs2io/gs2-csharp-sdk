...
namespace Gs2.Gs2Guild.Model.Cache
{
    public static class Gs2Guild
    {
        public static string PutUserData(
            ...
            switch (kind) {
                case "joinedGuild":
                    return Gs2.Gs2Guild.Model.JoinedGuild.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
                case "sendMemberRequest":
                    return Gs2.Gs2Guild.Model.SendMemberRequest.FromJson(JsonMapper.ToObject(payload))
                        ?.PutUserData(cache, namespaceName, userId, timeOffset);
            }
            return null;
        }
        ...
        public static bool SetListCached(
            ...
            switch (kind) {
                case "joinedGuild":
                    cache.SetListCached<Gs2.Gs2Guild.Model.JoinedGuild>(parentKey);
                    return true;
                case "sendMemberRequest":
                    cache.SetListCached<Gs2.Gs2Guild.Model.SendMemberRequest>(parentKey);
                    return true;
            }
            return false;
        }
