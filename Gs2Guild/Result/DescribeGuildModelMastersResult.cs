...
namespace Gs2.Gs2Guild.Result
{
	...
	public class DescribeGuildModelMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeGuildModelMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeGuildModelMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2Guild.Model.GuildModelMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2Guild.Model.GuildModelMaster.FromJson(v);
                }).ToArray())
