        ...
        public void Validate() {
            {
                if (TriggerScriptId.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("scriptSetting", "stateMachine.scriptSetting.triggerScriptId.error.tooLong"),
                    });
                }
            }
            {
                switch (DoneTriggerTargetType) {
                    ...
                    default:
                        throw new Gs2.Core.Exception.BadRequestException(new [] {
                            new RequestError("scriptSetting", "stateMachine.scriptSetting.doneTriggerTargetType.error.invalid"),
                        });
                }
            }
            {
                if (DoneTriggerScriptId.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("scriptSetting", "stateMachine.scriptSetting.doneTriggerScriptId.error.tooLong"),
                    });
                }
            }
            {
                if (DoneTriggerQueueNamespaceId.Length > 1024) {
                    throw new Gs2.Core.Exception.BadRequestException(new [] {
                        new RequestError("scriptSetting", "stateMachine.scriptSetting.doneTriggerQueueNamespaceId.error.tooLong"),
                    });
                }
            }
        }
