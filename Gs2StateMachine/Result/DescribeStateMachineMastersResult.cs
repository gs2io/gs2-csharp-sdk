...
namespace Gs2.Gs2StateMachine.Result
{
	...
	public class DescribeStateMachineMastersResult : IResult
	{
    	...
    	[Preserve]
#endif
        public static DescribeStateMachineMastersResult FromJson(JsonData data)
        {
            if (data == null) {
                return null;
            }
            return new DescribeStateMachineMastersResult()
                .WithItems(!data.Keys.Contains("items") || data["items"] == null || !data["items"].IsArray ? new Gs2.Gs2StateMachine.Model.StateMachineMaster[]{} : data["items"].Cast<JsonData>().Select(v => {
                    return Gs2.Gs2StateMachine.Model.StateMachineMaster.FromJson(v);
                }).ToArray())
